...
/* gen-5.57 — tre lavori:
   ...
   I pezzi stanno in h564/NNa.txt (vecchio) e h564/NNb.txt (nuovo) invece che
...
P.forEach(([a, b], i) => {
  ...
  /* le md5 dicono «il testo e' quello che volevo», non «il testo ha senso»:
     questo guarda che il pezzo nuovo lasci parentesi e tag JSX in pari come li
     lasciava il vecchio. E' il controllo che mi e' mancato su gen-5.64. */
  if (!bilancia(a, b, numeri[i])) ok = false;
...
writeFileSync("pairs564.json", JSON.stringify(P));
console.log("scritto app-dev.jsx e pairs564.json");
